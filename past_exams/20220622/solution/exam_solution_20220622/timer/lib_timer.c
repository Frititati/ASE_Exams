...
void timer_enable_interrupt(uint8_t timer_num){
	if(timer_num==0){
		LPC_TIM0->MCR |= 1;				/*clear on mr match */
		NVIC_EnableIRQ(TIMER0_IRQn);
	//NVIC_SetPriority(TIMER0_IRQn, 4);		/* less priority than buttons */
	NVIC_SetPriority(TIMER0_IRQn, 6);	 // merdina
	}else if (timer_num==1){
		LPC_TIM1->MCR |= 1;				
	...
	NVIC_SetPriority(TIMER2_IRQn, 7);		/* more priority than buttons */
	}else if(timer_num==3) {
		LPC_TIM3->MCR |= 1;				
		NVIC_EnableIRQ(TIMER3_IRQn);
	NVIC_SetPriority(TIMER3_IRQn, 8);		/* more priority than buttons */
	}


}

uint32_t timer_read_value(uint8_t timer_num) {
	if(timer_num==0){
		return LPC_TIM0->TC;
	}else if (timer_num==1){
	return LPC_TIM1->TC;
